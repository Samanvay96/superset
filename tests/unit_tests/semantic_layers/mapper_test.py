...
def test_coerce_integer_rejects_other_types() -> None:
    with pytest.raises(ValueError, match="Invalid integer value"):
        _coerce_scalar_filter_value([1], _dim(pa.int64()))  # type: ignore[arg-type,unused-ignore]
...
def test_coerce_floating_rejects_other_types() -> None:
    with pytest.raises(ValueError, match="Invalid numeric value"):
        _coerce_scalar_filter_value([1.0], _dim(pa.float64()))  # type: ignore[arg-type,unused-ignore]
